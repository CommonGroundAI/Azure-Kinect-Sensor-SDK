...
constexpr std::chrono::microseconds MAX_ALLOWABLE_TIME_OFFSET_ERROR_FOR_IMAGE_TIMESTAMP(1000);
...
static void log_lagging_time(const char *lagger, k4a::capture &master, k4a::capture &sub)
{
    std::cout << std::setw(6) << lagger << " lagging: mc:" << std::setw(6)
              ...
              << sub.get_color_image().get_device_timestamp().count() << "us diff:" 
              << sub.get_color_image().get_device_timestamp().count()- master.get_color_image().get_device_timestamp().count() << "\n";
}
